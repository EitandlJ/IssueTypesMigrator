...
class JiraAzureMigrator {
  constructor(authAzure, azureOrg, azureProject, authJiraEmail, authJiraToken) {
    if (!authJiraEmail || !authJiraToken) {
    ...
    this.jiraHeaders = {
      Authorization: `Basic ${Buffer.from(`${authJiraEmail}:${authJiraToken}`).toString('base64')}`
    };

    this.azureHeaders = {
      Authorization: `Basic ${Buffer.from(`:${authAzure}`).toString('base64')}`
    };

    this.azureOrg = azureOrg;
  ...
  async attachImageToWorkItem(workItemId, filePath) {
    ...
    try {
      ...
      console.log(`✅ Imagen adjunta correctamente al Work Item ${workItemId}`);
  ...
  /*/ 🆕 ✅ Convertir enlaces de video de Jira `!video.mp4|width=576,height=416!` a HTML <video> 
     body = body.replace(/!(.*?)\|(.*?)?!/g, (match, filename, attributes) => {
        const videoExt = filename.split('.').pop().toLowerCase();
        if (['mp4', 'avi', 'mov', 'mkv', 'webm'].includes(videoExt)) {
            const widthHeightMatch = attributes.match(/width=(\d+),height=(\d+)/);
            const width = widthHeightMatch ? widthHeightMatch[1] : '576'; // Default width
            const height = widthHeightMatch ? widthHeightMatch[2] : '416'; // Default height
            return `<video width="${width}" height="${height}" controls>
                        <source src="${filename}" type="video/${videoExt}">
